...
export const EN_PLAIN: Record<string, string> = {
  ...
  "清除该模型的自定义元数据与未保存内容": "Clear this model's custom metadata and unsaved content",
  "请先填写模型名称": "Enter a model name first",
  "当前模型名没有内置元数据可匹配": "The current model name has no built-in metadata to match",
  "该模型没有自定义元数据可清除": "This model has no custom metadata to clear",
  "清除该模型的自定义元数据与未保存内容，生成时改用内置": "Clear this model's custom metadata and unsaved content; generation will use the built-in data",
